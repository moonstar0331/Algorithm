...
# 주의사항 - 문제의 정답으로 가능한 거리의 최댓값보다 큰 값임을 보장해야 한다.
...
while Q:
    ...
    # 꺼낸 정보가 최신 정보랑 다르면, 의미없이 낡은 정보이므로 폐기
    if dist[x] != dist_x:
        continue

    # 연결된 모든 간선들을 통해서 다른 정보들에 대한 정보를 갱신
    for u, weight in con[x]:
        # u 까지 갈 수 있는 더 짧은 거리를 찾았다면 이에 대한 정보를 개신하고 PQ에 기록해준다.
